...
func InitColumnRoutes(router *fiber.Router, container *app.Container, cfg config.Server) {
	...
	columnGroup.Post("/board/:boardId", handlers.CreateColumn(container.ColumnService()))
	columnGroup.Get("/:id", handlers.GetColumnByID(container.ColumnService()))
	columnGroup.Get("/board/:boardId", handlers.GetAllColumns(container.ColumnService()))
